...
MAIN_SRCS = $(filter-out $(SRC_DIR)/DNNGuiUtils.cpp $(SRC_DIR)/CNNGuiUtils.cpp,$(SRC_FILES))
GUI_SRCS = $(SRC_FILES)
...
NON_MAIN_SRCS = $(filter-out $(SRC_DIR)/main.cpp $(SRC_DIR)/DNNGuiUtils.cpp $(SRC_DIR)/CNNGuiUtils.cpp,$(SRC_FILES))
...
LINT_FILES = $(shell git ls-files '*.cpp' '*.h' ':!:third_party/**' ':!:build/**' ':!:coverage/**' ':!:mnist/**')
	...
	# `make main` excludes GUI sources, so exclude them here as well.
