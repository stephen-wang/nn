...
MAIN_SRCS = $(filter-out $(SRC_DIR)/NNGuiUtils.cpp,$(SRC_FILES))
GUI_SRCS = $(SRC_FILES)
...
NON_MAIN_SRCS = $(filter-out $(SRC_DIR)/main.cpp $(SRC_DIR)/NNGuiUtils.cpp,$(SRC_FILES))
...
LINT_FILES = $(shell git ls-files '*.cpp' '*.h' ':!:third_party/**' ':!:build/**' ':!:coverage/**' ':!:mnist/**')
	...
	# `make main` excludes src/NNGuiUtils.cpp, so exclude it here as well.
TIDY_SRCS = $(filter-out $(SRC_DIR)/NNGuiUtils.cpp,$(MAIN_SRCS))
